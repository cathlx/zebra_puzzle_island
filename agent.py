...
class Agent:
    ...
    def visit_decision(self, distance_matrix: np.ndarray) -> tuple[int, int]:
        ...
        house_to_visit = random.choices(range(6), self.action_probs.visit_house)[0]
        if house_to_visit == self.at:
            return -1, np.nan # stays where is 
        
        eta = distance_matrix[self.at][house_to_visit]
        self.destination = house_to_visit
        return house_to_visit, int(eta) if not np.isnan(eta) else eta
    ...
    # def swap_decision(self) -> SwapDecision:
    #     return random.choices(list(SwapDecision), self.action_probs.swap_probs)[0]
